...
import subprocess
import warnings
import shutil  # Для which: поиск ffmpeg в PATH
import json  # Для ffprobe JSON
...
# Suppress pydub warnings (кросс-платформен)
warnings.filterwarnings('ignore', category=RuntimeWarning, module='pydub.utils')

# Поиск ffmpeg/ffprobe в PATH (Linux/Windows/Mac)
def find_ffmpeg():
    """Находит пути к ffmpeg/ffprobe. Если нет — инструкция."""
    ffmpeg_path = shutil.which('ffmpeg')
    ffprobe_path = shutil.which('ffprobe')
    if not ffmpeg_path:
        raise RuntimeError(
            "FFmpeg не найден в PATH. Установи: sudo apt install ffmpeg (Ubuntu) "
            "или brew install ffmpeg (Mac). Windows: choco install ffmpeg."
        )
    print(f"FFmpeg found: {ffmpeg_path}, executable: {os.access(ffmpeg_path, os.X_OK)}")
    print(f"FFprobe found: {ffprobe_path}, executable: {os.access(ffprobe_path, os.X_OK)}")
    
    # Тихий чек версии
    subprocess.run([ffmpeg_path, '-version'], capture_output=True, check=False)
    return ffmpeg_path, ffprobe_path

# Инициализируем пути (один раз)
